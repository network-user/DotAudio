...
Дата: 2026-09-06, каталог аналогов расширен 2026-09-07.
Внешние источники использованы для выбора направления; продуктовые решения
DotAudio не являются копией контрактов других приложений.

## Аналоги

Полный каталог со ссылками, статусами «есть / взять / не брать» и очередью
адаптации: [ANALOGS.md](ANALOGS.md).

Краткая выжимка, с которой начинали:
...
| [cjpais/Handy](https://github.com/cjpais/Handy) | Локальная OSS-диктовка, hold/toggle, GigaAM для русского |
| [LiveTranslate](https://github.com/Dehydrated-gumarabic195/LiveTranslate) | WASAPI + Silero + click-through overlay |
...
Вывод: общая идея - короткий путь от голоса до текста. Для DotAudio к нему
добавляются live-зал, редактор медиа и наблюдение за эфиром.
Автоматическую «умную редактуру» через LLM не добавлять по умолчанию:
она меняет сказанное, повышает задержку и требует отдельной настройки.
