...
// скорость, звук, волна peaks и воспроизведение одной фразы.
Rectangle {
    id: root
    ...
    Popup {
        ...
        parent: root
        x: {
            var pos = rateBtn.mapToItem(root, 0, 0)
            return Math.max(8, Math.min(pos.x, root.width - implicitWidth - 8))
        }
        y: {
            var below = rateBtn.mapToItem(root, 0, rateBtn.height).y + 6
            if (below + implicitHeight <= root.height - 8)
                return below
            return Math.max(8, rateBtn.mapToItem(root, 0, 0).y - implicitHeight - 6)
        }
        padding: 6
        modal: false
        focus: true
