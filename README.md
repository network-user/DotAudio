...
- Переносимый мини-остров: уровень микрофона, статус записи, раскрытие в
  рабочее окно, горячая клавиша `Ctrl+Alt+O`.
- Live-субтитры - главный режим. Русский язык выбран по умолчанию; режим
  Whisper Translate создаёт английские субтитры.
- Диктовка с копированием текста в буфер обмена и сохранением в историю.
...
- Локальная SQLite-история, журнал работы, модели `tiny`, `base`, `small`,
...
src/dotaudio/
  ...
  storage.py, transcripts.py          # SQLite и экспорт расшифровок
...
Последняя проверка: 40 passed, `ruff check src tests` без ошибок, QML smoke test
